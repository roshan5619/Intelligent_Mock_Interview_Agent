...
\definecolor{blockbg}{HTML}{121627}     % subtle lift over slidebg (used for blocks + boxes)
\definecolor{codebgDark}{HTML}{0F1322}   % code-block background
...
\setbeamercolor{block body}{bg=blockbg, fg=slidefg}
...
%%-------- TikZ helpers --------%%
%% Note: TikZ's `font=` key takes font commands only. Colour goes via `text=`.
\tikzset{
  block/.style={draw=brand!60, rounded corners=4pt, fill=brand!12, text=slidefg,
                minimum width=3.0cm, minimum height=0.9cm,
                font=\sffamily\footnotesize, align=center,
                blur shadow={shadow blur steps=4}},
  store/.style={draw=accent!60, rounded corners=2pt, fill=accent!12, text=slidefg,
                minimum width=2.6cm, minimum height=0.8cm,
                font=\sffamily\scriptsize, align=center},
  ext/.style={draw=ok!60, rounded corners=4pt, fill=ok!15, text=slidefg,
  ...
  badge/.style={fill=brand, rounded corners=4pt, inner sep=4pt, text=white,
...
\newcommand{\kpi}[2]{%
  \begin{tikzpicture}
    \node[draw=brand!60, fill=brand!10, rounded corners=8pt, minimum width=3.6cm, minimum height=1.6cm,
      ...
      \color{white}\bfseries\Large #1 \par
      \color{slidefg!75}\scriptsize #2%
    };
  \end{tikzpicture}}

\title{IPHIPI -- Agentic AI Mock Interview Platform}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.32\textwidth}
      \begin{tikzpicture}
        \node[draw=brand!60,fill=brand!10,rounded corners=8pt,inner sep=10pt,
              text width=4.5cm,align=left,text=slidefg,font=\sffamily\footnotesize]{
          \biglabel{Static} \par\vspace{4pt}
    ...
    \column{0.32\textwidth}
      \begin{tikzpicture}
        \node[draw=brand!60,fill=brand!10,rounded corners=8pt,inner sep=10pt,
              text width=4.5cm,align=left,text=slidefg,font=\sffamily\footnotesize]{
          \biglabel{Text-only} \par\vspace{4pt}
    ...
    \column{0.32\textwidth}
      \begin{tikzpicture}
        \node[draw=brand!60,fill=brand!10,rounded corners=8pt,inner sep=10pt,
              text width=4.5cm,align=left,text=slidefg,font=\sffamily\footnotesize]{
          \biglabel{Opaque} \par\vspace{4pt}
...
\begin{frame}{End-to-end user journey}
  \vspace{0.2cm}
  %% `step` collides with TikZ's built-in `step=<value>` grid key, so we name
  %% our local style `flowstep`. Same fix applied to the per-turn flow slide.
  \begin{tikzpicture}[
    node distance=2mm and 6mm,
    flowstep/.style={draw=brand!60, rounded corners=4pt, fill=brand!10,
                     text=slidefg, minimum width=2.3cm, minimum height=1.2cm,
                     font=\sffamily\scriptsize, align=center,
                     blur shadow={shadow blur steps=3}}
    ]
    \node[flowstep] (s1) {Upload\\resume};
    \node[flowstep, right=of s1] (s2) {Agent 1\\ context profile};
    \node[flowstep, right=of s2] (s3) {JD-match\\ rank IPHIPI roles};
    \node[flowstep, right=of s3] (s4) {Apply\\ see fit score};
    \node[flowstep, right=of s4] (s5) {Mock\\ interview};
    \node[flowstep, right=of s5] (s6) {Live\\ multimodal scoring};
    \node[flowstep, right=of s6] (s7) {Coaching\\ report};
...
\begin{frame}{What happens in one interview turn}
  ...
  \begin{tikzpicture}[
    ...
    flowstep/.style={draw=brandLite!40, rounded corners=3pt, fill=blockbg,
                     text=slidefg, font=\sffamily\scriptsize,
                     minimum width=3.0cm, minimum height=0.7cm, align=center},
    para/.style={draw=brand!60, fill=brand!15, rounded corners=3pt,
                 text=white, font=\sffamily\scriptsize\bfseries,
                 minimum width=2.8cm, minimum height=0.7cm, align=center}
    ]
    \node[flowstep] (s1) {Browser flushes metrics};
    \node[flowstep, below=of s1] (s2) {POST \texttt{/api/interview/turn}};
    \node[flowstep, below=of s2] (s3) {Persist candidate turn};
    ...
    \node[flowstep, below=8mm of s3] (s4) {Persist all evaluations};
    \node[flowstep, below=of s4] (s5) {Build 2-turn trends};
    \node[flowstep, below=of s5] (s6) {Agent 2 decides next turn\\\emph{heuristics may override}};
    \node[flowstep, below=of s6] (s7) {Return JSON $+$ TTS};
